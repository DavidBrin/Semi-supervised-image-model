...
def evaluate_models(dataset, unet, vit, ensemble, device="cpu"):
    ...
    for i in range(len(dataset)):
        image, mask = dataset[i] # mask is likely (512, 512)
        target_h, target_w = mask.shape[-2:]
        
        image_batch = image.unsqueeze(0).to(device)
        target = mask.cpu().numpy()

        # 1. Get raw predictions (as tensors)
        # Assuming your predict functions return Tensors. 
        # If they return numpy, you'll need to wrap them in torch.from_numpy() to use interpolate
        pred_unet_t = predict_unet(unet, image_batch, device=device)
        pred_vit_t = predict_vit(vit, image_batch, device=device)
        pred_ens_t = ensemble.predict_mask(image_batch) # Assuming this returns a tensor
        ...
        for name, pred_tensor in preds_dict.items():
            # Ensure it's a tensor (just in case your predict function returns numpy)
            if isinstance(pred_tensor, np.ndarray):
                pred_tensor = torch.from_numpy(pred_tensor)

            # 2. Resize prediction to match ground truth target shape
            if pred_tensor.shape[-2:] != (target_h, target_w):
                # Ensure the tensor is 4D: (1, 1, H, W)
                # We use a loop to unsqueeze until we hit 4 dims
                temp_tensor = pred_tensor
                while temp_tensor.ndim < 4:
                    temp_tensor = temp_tensor.unsqueeze(0)
                
                # Perform interpolation
                temp_tensor = F.interpolate(
                    temp_tensor.float(), # Some versions require float input
                    size=(target_h, target_w), 
                    mode='nearest'
                )
                pred_tensor = temp_tensor.squeeze() # Back to (target_h, target_w)
            # 3. Convert to numpy for your metric functions
